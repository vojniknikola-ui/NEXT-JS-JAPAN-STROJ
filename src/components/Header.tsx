...
const Header: React.FC<HeaderProps> = ({ activePage, setActivePage, cartItemCount: propCartItemCount }) => {
  ...
  return (
    <header className="bg-[#050505]/95 backdrop-blur-sm text-white shadow-lg shadow-black/20 sticky top-0 z-50 border-b border-white/5">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          <div className="flex-shrink-0">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-semibold tracking-tight cursor-pointer text-[#ff6b00]" onClick={() => handleNavClick('home')}>
          ...
          <div className="flex items-center">
            <nav className="hidden md:flex items-center space-x-1 lg:space-x-2">
              {navItems.map((item) => (
                <button
                  ...
                  className={`relative px-3 lg:px-4 py-2 rounded-full text-xs lg:text-sm font-semibold uppercase tracking-wide transition-all duration-200 ${
                    activePage === item.id
            ...
            <div className="hidden md:flex items-center space-x-2 lg:space-x-3 ml-4 lg:ml-6 pl-4 lg:pl-6 border-l border-white/10">
                 ...
                 <a href="https://olx.ba" target="_blank" rel="noopener noreferrer" className="text-neutral-400 hover:text-[#ff6b00] transition-all duration-300 hover:scale-110 flex items-center gap-1 lg:gap-2">
                     <OlxIcon className="h-5 w-5 lg:h-6 lg:w-6" />
                     <span className="text-xs lg:text-sm hidden xl:inline">OLX</span>
                 </a>
            </div>
          </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#050505]/95 border-t border-white/10 flex justify-around p-1.5 z-50 backdrop-blur-sm">
        {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleNavClick(item.id as Page)}
              className={`relative flex flex-col items-center justify-center w-1/6 p-1.5 rounded-lg transition-colors duration-200 min-w-fit ${
                activePage === item.id ? 'text-[#ff6b00]' : 'text-neutral-400'
              }`}
            >
              <item.icon className="h-5 w-5 sm:h-6 sm:w-6" />
              <span className="text-[10px] sm:text-xs mt-0.5 text-center font-medium leading-tight">{item.label}</span>
              {item.id === 'cart' && cartItemCount > 0 && (
